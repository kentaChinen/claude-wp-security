...
### 0-0: ナレッジベースの参照（既知パターン・IOCの確認）

過去の検査で蓄積された情報を最初に確認する。2回目以降の検査では特に重要。

```bash
THREAT_DIR="$(pwd)/.claude/threat-intel"

if [ -d "$THREAT_DIR" ] && find "$THREAT_DIR" -name "*.md" | xargs grep -l "^|" 2>/dev/null | head -1 | grep -q .; then
  echo "=== [ナレッジベース] 既知のコードパターン ==="
  cat "$THREAT_DIR/patterns.md" 2>/dev/null

  echo "=== [ナレッジベース] 既知の IOC（URL・IP・ドメイン・ハッシュ） ==="
  cat "$THREAT_DIR/ioc.md" 2>/dev/null

  echo "=== [ナレッジベース] 既知の攻撃手法 ==="
  cat "$THREAT_DIR/techniques.md" 2>/dev/null

  echo "=== [ナレッジベース] メモ ==="
  cat "$THREAT_DIR/notes.md" 2>/dev/null
else
  echo "ナレッジベース未記録（初回検査、または未使用）"
fi
```

ナレッジベースに記録がある場合は、**既知のパターンや IOC を念頭に置きながら** 以降の各フェーズを進めること。
特に IOC に記録されたドメイン・URL・ハッシュが今回のサイトにも混入していないか重点的に確認する。
...
- `/memo add pattern [パターン]` — 新しいコードパターンを記録
- `/memo add ioc [値]` — 新しい IOC（URL・IP・ドメイン）を記録
- `/memo add technique [手法]` — 新しい攻撃手法を記録
- `/memo promote` — 重要な発見を CLAUDE.md・スキルファイルへ昇格
```
