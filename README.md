...
検査中に発見したパターン・IOC・攻撃手法は `/memo` でナレッジベースに蓄積でき、次回以降の検査に自動反映されます。
...
/memo [add|show|search|promote]  # ナレッジベースの記録・検索・昇格
```
...
| `/memo` | 発見したパターン・IOC・手法をナレッジベースに記録・検索・各ファイルへ昇格 |
...
検査中に発見した情報を `.claude/threat-intel/` に蓄積し、次回以降の検査に活かします。

```
/memo add pattern eval(str_rot13(...))   # コードパターンを記録
/memo add ioc malicious-cdn.ru           # 外部ドメイン・IP を記録
/memo add technique WebShell配置手口     # 攻撃手法を記録
/memo add note wp-login.phpが消えていた  # その他メモ
/memo show                               # 全メモを一覧表示
/memo search eval                        # キーワード検索
/memo promote                            # CLAUDE.md・スキルファイルへ昇格
```

`/security-audit` と `/malware-scan` は起動時にナレッジベースを自動参照します。
...
```
STEP 1. 感染サーバーから テーマ / DB / uploads をダウンロード
          ↓
STEP 2. ローカルで検査・クリーンアップ
          /security-audit  → テーマファイル検査
          /db-scan         → DB 検査
          /uploads-scan    → uploads 検査
          /memo add        → 発見したパターン・IOC を随時記録
          ↓
STEP 3. 再検査（全ファイルを改めて確認）
          ↓
STEP 4. 新サーバーへアップ
          ...
          ↓
STEP 5. DNS 切り替え・WAF 導入・最終確認
          /memo promote → 重要な発見を各ファイルへ昇格
```
